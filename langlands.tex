...
\begin{proof}[Proof of Theorem \ref{thm:langcorr2}.]
    ...
    \begin{enumerate}[(1)]
        ...
        where $\chi\phi$ are simultaneously viewed as characters of $F^\times$ and $\mathcal{W}_F$ as usual, and $\chi_1(x)=\phi(x)|x|^{1/2}$ and $\chi_2(x)=\phi(x)|x|^{-1/2}$ as above. The second equality comes from Figure 1.
        \item Finally, if $\pi$ is reducible and $\nn\neq0$, then $\bm\pi^1(\chi\rho,V,\nn)=\chi\phi\cdot\St_G=\chi\bm\pi^1(\rho,V,\nn)$ and
        $$L(\chi\bm\pi^1(\rho),s)=L(\chi\phi\cdot\St_G,s)=L\left(\chi\phi,s+\frac{1}{2}\right)=L\left(\chi\chi_1,s\right)=L(\chi\otimes\rho_\nn,s)=L((\chi\rho,V,\nn),s),$$
        where in this last case we are keeping track of the nilpotent $\nn$. The second equality comes from Figure 1.
    \end{enumerate}
    To prove that $\bm\pi^1$ satisfies the property in the local constants, one needs a considerable amount of work. One uses the local constants listed in Figure \ref{table:Lfunc} and a better understanding of local constants associated to Weil--Deligne representations. See, for example, \cite[Sections 29-31]{BH1}. We omit this from the proof.
\end{proof}
