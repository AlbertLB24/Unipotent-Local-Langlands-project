...
\begin{rem}\label{BHThm2}
    Similarly to case of the $L$-functions, there are more general versions of the functional equation and the local constant that hold for any irreducible smooth representation $\pi$ of $G$. More concretely, for any irreducible smooth representation of $G$. There is a unique rational function $\gamma(\pi,s,\psi) \in \CC(q^{-s})$ such that 
...
Similarly to our discussion with $L$-functions, we do not have the tools to prove the above result if $\pi$ is a cuspidal representation. However, with the work we have done so far, we can easily calculate $\gamma(\pi,s,\chi)$ whenever $\pi$ is a principal series representation.
...
\begin{proof}
    Similarly to the proof of the $L$-function for $\pi$, we use the fact that $\mathcal{C}(\pi)\subseteq\mathcal{C}(\Sigma)$, so $\mathcal{Z}(\pi)\subseteq\mathcal{Z}(\Sigma)$. In particular, the convergence for the zeta functions also hold for $\pi$. Therefore, for any $\Phi\in C_c^\infty(A)$ and $f\in\mathcal{C}(\pi)$ we have that $\check{f}\in\mathcal{C}(\check{\pi})\subseteq\mathcal{C}(\check{\Sigma})$ and
...
Similarly to the $\GL_1(F)$ case in the previous chapter, it is natural to consider the terms $\frac{\zeta(\Phi,f,s+1/2)}{L(\pi,s)}\in\CC[q^{-s},q^s]$ that motivate the analogous definition to \ref{defn:localconstant}.
...
$$\frac{\zeta(\hat{\Phi},\check{f},3/2-s)}{L(\check{\pi},1-s)} = \varepsilon(\pi,s,\psi) \frac{\zeta(\Phi,f,s+1/2)}{L(\pi,s)},$$
and the local constant in this setting also satisfies a functional equation. 
...
\end{cor}
\begin{proof}
    The first statement comes from the Fourier inversion formula and Remark \ref{BHThm2}. The $\omega_\pi(-1)$ term comes from the minus sign in $\hat{\hat{\Phi}}(x)=\Phi(-x)$ and the observation that for a matrix coefficient $f \in \mathcal C(\pi)$ we have $f(-g)=\omega_\pi(-1)f(g)$. The functional equation and Remark \ref{BHThm1} implies that $\varepsilon$ is a unit in $\CC[q^{-s},q^s]$, and the units are precisely the elements of the form $aq^{bs}$ for $b \in \ZZ$.
\end{proof}
...
The Propositions \ref{prop:gl2factor} and \ref{prop:gl2gamma} provide a description of the $L$-function and the $\gamma(\pi,s,\psi)$ factor in the case that $\pi = \iota_B^G \chi$ and the list at the end of Section \ref{sec:LfuncGL2} and Lemma \ref{lem:gamma} describe them when $\pi$ is a principal series representation. From this information, one can derive formulas for the local constants that we omit (only the local constant for the Steinberg representation is a non-trivial computation). As we will see in the following section, principal series representations are completely determined by its $L$-function. The results for all principal series representations are summarized in the following table:

\iffalse
As in Theorem \ref{classify}, the representations $\pi = \iota_B^G \chi$ are typically irreducible - they are only reducible when $\chi = \phi \delta_B^{\pm 1/2}$ for some character $\phi$ of $F^\times$. In this case the composition factors are characters $\phi \circ \det$, and twists of Steinberg $\phi \mathrm{St}_G$. We state without proof the $L$-functions and local constants in the case that $\pi$ is one of these composition factors. For more detail see Sections 26.5 - 26.8 of \cite{BH1}. 
\fi
