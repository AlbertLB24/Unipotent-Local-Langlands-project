...
Note that $z_m=z_m(\Phi,\chi)$ vanish for $m \ll0$ because $\Phi$ is compactly supported on $F$, so $\supp\Phi\subseteq\pp^N$ for some sufficiently small $N$. In this way it is clear that $\zeta(\Phi,\chi,s) \in \CC((q^{-s}))$. 
...
\begin{notn}
    Let $\chi$ be a character of $F^\times$. Then we define 
...
\begin{defn}
    Given characters $\chi$ and $\psi$ of $F^\times$ and $F$ respectively, define
    $$\varepsilon(\chi,s,\psi) := \gamma(\chi,s,\psi) \cdot \frac{L(\chi,s)}{L(\check{\chi},s)},$$
    denoted as \textit{Tate's local constant}.
\end{defn}
...
\end{cor}
\begin{proof}
    The first statement comes from the Fourier inversion formula, where the $\chi(-1)$ term comes from the minus sign in $\hat{\hat{\Phi}}(x) = \Phi(-x)$. The functional equation implies that $\varepsilon$ is a unit in $\CC[q^{-s},q^s]$, and the units are precisely the elements of the form $aq^{bs}$ for $a\in\CC^\times$ and $b \in \ZZ$.
\end{proof}
